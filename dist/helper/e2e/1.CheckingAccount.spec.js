...
describe("A checking account", () => {
    ...
    it("allows a withdrawal when the withdrawal amount is less than the current available balance  (Checking Account Requirement #3)", () => {
        ...
        let withdrawAttempt = checking.withdrawMoney(500, "withdrawal test 1", TransactionOrigin_1.TransactionOrigin.BRANCH);
    ...
    it("disallows a withdrawal for more than the current available balance (Checking Account Requirement #4)", () => {
        let checking = createCheckingAccount();
        let balance = checking.balance;
        let overdraftAttempt = checking.withdrawMoney(balance + 1, "withdrawal test 1", TransactionOrigin_1.TransactionOrigin.BRANCH);
        ...
        let withdrawAttempt = checking.withdrawMoney(500, "withdrawal test 2", TransactionOrigin_1.TransactionOrigin.BRANCH);
    ...
    it("allows virtually an unlimited number of withdrawals (1000) of any type as long as enough balance is available (Checking Account Requirement #5)", () => {
        let checking = createCheckingAccount();
        const transactionTypes = [TransactionOrigin_1.TransactionOrigin.BRANCH, TransactionOrigin_1.TransactionOrigin.WEB, TransactionOrigin_1.TransactionOrigin.PHONE];
...
function createCheckingAccount() {
    return AccountFactory_1.AccountFactory.getCheckingAccountObject(new Date(2000, 0, 1));
}
//# sourceMappingURL=1.CheckingAccount.spec.js.map